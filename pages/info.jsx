...
const INFO_PAGES = {
  ...
  privacy: {
    ...
    body: [
      ...
      { heading: "アクセス解析ツールの利用", text: "当サイトでは、サイト改善や利用状況の把握のため、Google Analytics等のアクセス解析ツールを利用する場合があります。これらのツールはCookie等を使用し、閲覧ページ、利用端末、ブラウザ、参照元などの情報を取得することがありますが、氏名、メールアドレス、電話番号、具体的な相談内容など、フォームに入力された個人情報をアクセス解析ツールへ送信しないよう配慮します。" },
      { heading: "Cookieの利用", text: "当サイトでは、アクセス解析、利便性向上、サイト改善のためCookieを使用する場合があります。ブラウザの設定によりCookieを無効化できますが、一部機能が正しく動作しない場合があります。" },
